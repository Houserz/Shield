...
```bash
conda run -n miniproject1 python tools/pc_viewer.py --serial /dev/cu.usbmodem1101    # macOS
conda run -n miniproject1 python tools/pc_viewer.py --serial COM7                    # Windows
conda run -n miniproject1 python tools/pc_viewer.py --serial /dev/ttyACM0            # Linux
```
...
```bash
conda run -n miniproject1 python tools/pc_viewer.py --tcp 192.168.4.1
```
...
- Saves every received frame to `pc_runs/RUN_<timestamp>/stream.bin`.
  This is the PC-side live mirror of the same clean/noisy/denoised values written to SD;
  vector SD records are expanded to one stream packet per axis.
- Per-channel **live ring** overlays clean, noisy, and denoised samples, plus
...
dt = np.dtype([
    ...
    ('sid',    'u1'),
    ('axis',   'u1'),
    ('kind',   'u1'),   # 0=clean, 1=noisy, 2=denoised
    ('flags',  'u1'),
    ('reserved', '<u2'),
    ('seq',    '<u4'),
    ('ts_ms',  '<u4'),
...
mask = (arr['sid'] == 9) & (arr['axis'] == 1) & (arr['kind'] == 2)  # denoised accel.x
