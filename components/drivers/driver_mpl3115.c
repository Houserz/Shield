 ...
 *
 * Implementation per SparkFun MPL3115A2 Hookup Guide and NXP MPL3115A2
 * datasheet:
 *  - I2C address: 0x60 (7-bit)
 *  - WHO_AM_I register 0x0C, value 0xC4
 *  - Barometer mode: CTRL_REG1 (0x26) ALT=0; setModeStandby() before changing
 * CTRL1, then setModeActive()
 *  - setOversampleRate(0..7) → 1..128 samples; datasheet recommends 128
 * (OSR=7), 512 ms per reading
 *  - enableEventFlags(): PT_DATA_CFG (0x13) = 0x07 (required during setup)
 *  - Pressure: read OUT_P_MSB (0x01), OUT_P_CSB (0x02), OUT_P_LSB (0x03);
 * 20-bit, resolution 0.25 Pa
 *  - readPressure() returns barometric pressure in Pa (output converted to kPa
 * in this driver)
 *  - Uses I2C repeated start; poll STATUS (0x00) for data ready (bit 0x08) then
 * read OUT_P.
 *
 * Hardware: I2C (shared bus with MCP9808). Typical accuracy ±0.05 kPa, altitude
 * ±0.3 m, temp ±3 °C.
 */
...
// CTRL_REG1: SBYB=0 standby, SBYB=1 active; ALT=0 barometer, ALT=1 altimeter;
// OSR[2:0] oversample
...
// Pressure: 20-bit, 1 LSB = 0.25 Pa (datasheet Section 9.3, barometer
// resolution 0.25 Pa)
...
static esp_err_t mpl3115_write_reg(i2c_port_t port, uint8_t dev_addr,
...
static esp_err_t mpl3115_read_reg(i2c_port_t port, uint8_t dev_addr,
                                  uint8_t reg, uint8_t* value) {
  return i2c_master_write_read_device(port, dev_addr, &reg, 1, value, 1,
                                      pdMS_TO_TICKS(100));
}

static esp_err_t mpl3115_read_bytes(i2c_port_t port, uint8_t dev_addr,
...
bool mpl3115_init(SensorContext_t* ctx) {
  ...
  err = mpl3115_write_reg(port, addr, MPL3115_REG_PT_DATA_CFG,
                          MPL3115_PT_DATA_CFG_ENABLE);
  if (err != ESP_OK) {
...
/**
 * @brief Trigger a one-shot conversion (called from init and after each
 * successful read)
 */
...
bool mpl3115_read_sample(SensorContext_t* ctx, float* data_out) {
  ...
  uint8_t p_buf[3];
  err = mpl3115_read_bytes(port, addr, MPL3115_REG_OUT_P_MSB, p_buf,
                           sizeof(p_buf));
  if (err != ESP_OK) {
    return false;
  }
  ...
  uint32_t raw =
      ((uint32_t)p_buf[0] << 12) | ((uint32_t)p_buf[1] << 4) | (p_buf[2] >> 4);
