...
bool inmp441_init(SensorContext_t* ctx) {
  ...
  if (sample_rate < 7800 || sample_rate > 50000) {
    ESP_LOGW(TAG,
             "INMP441: sample_rate %d out of spec (7.8–50 kHz), using 16000",
             sample_rate);
    sample_rate = 16000;
  }
  ...
  i2s_chan_config_t chan_cfg =
      I2S_CHANNEL_DEFAULT_CONFIG((i2s_port_t)cfg->i2s_port, I2S_ROLE_MASTER);
  ...
  // STD config: Philips I2S, 24-bit, stereo (read both L/R - breakout L/R pin
  // varies) Datasheet: "64 SCK cycles per WS stereo frame", "MSB delayed by one
  // SCK" → Philips
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "INMP441: i2s_channel_init_std_mode failed %s",
             esp_err_to_name(ret));
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "INMP441: i2s_channel_enable failed %s",
             esp_err_to_name(ret));
...
bool inmp441_read_sample(SensorContext_t* ctx, float* data_out) {
  ...
  // Read one block (~1 ms @ 16 kHz): INMP441_READ_BLOCK samples (stereo frame,
  // we use left) 24-bit in 32-bit slot: 4 bytes per sample
  uint32_t buf[INMP441_READ_BLOCK];
  size_t bytes_read = 0;

  esp_err_t ret = i2s_channel_read(s_inmp441.rx_handle, buf, sizeof(buf),
  ...
  // Stereo: buf = [L0,R0,L1,R1,...]; use max(L_rms, R_rms) - INMP441 L/R pin
  // varies by breakout
