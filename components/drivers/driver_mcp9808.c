 ...
 *  - Temperature register TA: 0x05, 13-bit signed temperature, resolution
 * 0.0625°C
 *  - Manufacturer ID: register 0x06, value should be 0x0054
 *  - Device ID/version: register 0x07, value should be 0x0400
 *  - Resolution register: 0x08, recommended value 0x03 (0.0625°C, Adafruit
 * default)
 *
...
// Resolution setting (lowest 2 bits), 0x03 -> 0.0625°C, Adafruit example
// default config
...
// To share I2C bus with MPL3115, only configure and install I2C driver on first
// initialization Support for up to I2C_NUM_0 / I2C_NUM_1, indexed by i2c_port
...
static esp_err_t mcp9808_read_u16(i2c_port_t port, uint8_t dev_addr,
  ...
  esp_err_t err = i2c_master_write_read_device(
      port, dev_addr, &reg, 1, data, sizeof(data), pdMS_TO_TICKS(100));
  if (err != ESP_OK) {
    return err;
  }
 ...
 * 5. Ensure operating in continuous conversion mode (CONFIG register default
 * 0x0000 means continuous)
 *
...
bool mcp9808_init(SensorContext_t* ctx) {
  ...
  uint8_t addr =
      cfg->device_addr;  // Should default to 0x18, user can reconfigure
  ...
  //         .master.clk_speed = 400000,  // 400kHz, supported by MCP9808 &
  //         Adafruit example .clk_flags = 0,
  //     };
  ...
  //     ESP_LOGI(TAG, "I2C port %d initialized for MCP9808 (SDA=%d SCL=%d
  //     addr=0x%02X)",
  ...
  if (manuf_id != MCP9808_EXPECTED_MANUF_ID ||
      device_id != MCP9808_EXPECTED_DEVICE_ID) {
    ESP_LOGE(TAG,
             "MCP9808 ID mismatch: manuf=0x%04X (exp 0x%04X), dev=0x%04X (exp "
             "0x%04X)",
             manuf_id, MCP9808_EXPECTED_MANUF_ID, device_id,
             MCP9808_EXPECTED_DEVICE_ID);
    return false;
  }

  ESP_LOGI(TAG, "MCP9808 detected OK (manuf=0x%04X dev=0x%04X)", manuf_id,
           device_id);

  // ========== 4. Set resolution register (0.0625°C) ==========
  uint8_t res_value = MCP9808_RESOLUTION_0_0625C;
  err = i2c_master_write_to_device(
      port, addr, (uint8_t[]){MCP9808_REG_RESOLUTION, res_value}, 2,
      pdMS_TO_TICKS(100));
  if (err != ESP_OK) {
  ...
  // CONFIG register bit8 (SHDN) = 0 means continuous conversion. Default on
  // power-up is 0x0000, explicitly write it here for consistency.
...
bool mcp9808_read_sample(SensorContext_t* ctx, float* data_out) {
  ...
  uint8_t buf[2] = {0};
  esp_err_t err = mcp9808_read_bytes(port, addr, MCP9808_REG_AMBIENT_TEMP, buf,
                                     sizeof(buf));
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to read temperature (err=%d)", err);
    vTaskDelay(pdMS_TO_TICKS(
        50));  // brief delay before next read attempt to avoid bus congestion
    return false;
  }
