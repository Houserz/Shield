 ...
 *
 * Implements SHTP (Sensor Hub Transport Protocol) per Adafruit BNO085
 * datasheet. Hardware interface: SPI (requires CS, INT, RST pins) Reference:
...
static void shtp_build_packet(uint8_t channel, uint8_t seq,
  ...
  out_buf[1] =
      (uint8_t)((total_len >> 8) & 0x7F);  // bit 15 = 0 (no continuation)
  out_buf[2] = channel;
  out_buf[3] = seq;
...
// ==================== SPI Transfer ====================
static bool bno085_spi_transfer(const uint8_t* tx_buf, uint8_t* rx_buf,
                                size_t len) {
...
// ==================== FIX: Read entire packet in a SINGLE SPI transaction. The
// old code split header/payload into two SPI transactions, causing CS to toggle
// mid-packet. BNO085 interprets each CS assertion as a separate SHTP exchange,
// so the split read consumed TWO packets per call (evidence: sequence numbers
// incremented by 2 instead of 1 in debug logs).
static int bno085_read_packet(uint8_t* buf, size_t buf_size) {
  ...
  //     BNO085 needs valid host headers to advance its SHTP state machine
  //     during initialization. Sending all-zeros stalls the state machine
  //     (confirmed by logs).
  ...
  //     Sending [04 00 00 00] at runtime triggers ch=0 acknowledgment
  //     responses, which re-assert INT and create an infinite read loop
  //     (confirmed by logs).
  ...
  if (!s_bno085.initialized) {
    tx_dummy[0] =
        0x04;  // Init phase: valid empty SHTP header to drive state machine
  }
...
// data before sending (INT is HIGH), there's no simultaneous BNO085 data to
// cover.
...
// Format: ReportID(1) + Seq(1) + Status(1) + Delay(1) + X(2) + Y(2) + Z(2) = 10
// bytes X,Y,Z are int16, Q8.2 -> value/100 = m/s²
...
bool bno085_init(SensorContext_t* ctx) {
  ...
  if (cfg->cs_pin < 0 || cfg->sclk_pin < 0 || cfg->mosi_pin < 0 ||
      cfg->miso_pin < 0) {
    ESP_LOGW(TAG,
             "BNO085: Pin config incomplete (cs=%d sclk=%d mosi=%d miso=%d)",
  ...
  // (uninitialized) when BNO085 boots, its SPI slave may enter an undefined
  // state, causing all subsequent SPI reads to return invalid data.
  ...
  // Retry loop: BNO085 boot timing is variable; production libraries retry up
  // to 3 times. Each attempt does a hardware reset + drain + product ID
  // request. No soft reset needed - hardware reset is sufficient and more
  // reliable.
  ...
  for (int attempt = 0; attempt < MAX_INIT_ATTEMPTS; attempt++) {
    ...
    // produces a limited set of init packets (advertisement, reset, init
    // response). Without the soft reset, hardware-reset-only produces a massive
    // FRS data dump (80+ packets) that buries the Product ID response
    // (confirmed by logs).
    uint8_t reset_cmd = 1;
    ...
    //   2. After first valid packet, embed Product ID request in next read's TX
    //   buffer
    //      (full 512-byte transaction ensures reliable delivery even when
    //      BNO085 has pending data - a separate 6-byte send fails
    //      intermittently)
    ...
    for (int drain_i = 0; drain_i < 80; drain_i++) {
      ...
      // Embed Product ID request in the next read transaction (after first
      // valid pkt)
      if (!pid_request_sent && drain_count >= 1) {
        ...
        uint16_t rx_len =
            (uint16_t)rx_buf[0] | ((uint16_t)(rx_buf[1] & 0x7F) << 8);
  ...
  // Parse and log product ID (use saved_pid_buf since rx_buf may have been
  // overwritten by drain)
  ...
  // The BNO085 sends many ch=0 FRS records after reset; they must be consumed
  // first.
  int frs_count = 0;
  ...
  *(uint32_t*)(set_feature + 13) = 0;
  ...
  bno085_send_packet(SHTP_CHANNEL_CONTROL, &s_bno085.seq_control, set_feature,
                     17);
  vTaskDelay(pdMS_TO_TICKS(50));
