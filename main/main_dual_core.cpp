 ...
 * TODO: Modify acquisition duration, trigger method, etc. based on actual
 * requirements
 */
extern "C" void app_main(void) {
  ...
  const run_session_t* session = sd_get_current_session();
  ESP_LOGI(TAG, "Run session created: %s at %s", session->run_id,
           session->run_path);
  ...
  // Continue to run data acquisition until button press or RUN_DURATION
  // hours/minutes have elapsed NOTE:
  //  - To collect data for RUN_DURATION **minutes**, use MS_PER_MIN
  //  - To collect data for RUN_DURATION **hours**, use MS_PER_HR
  ...
  ESP_LOGI(TAG,
           "Acquisition STOP at %" PRIu32 " ms since boot (ran %" PRIu32
           " ms = %.2f hours)",
  ...
  ESP_LOGI(TAG,
           "Stats: fast=%" PRIu32 " medium=%" PRIu32 " slow=%" PRIu32
           " overruns=%" PRIu32 " sd_errors=%" PRIu32,
           statistics.fast_samples, statistics.medium_samples,
